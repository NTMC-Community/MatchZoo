...
def test_hinge_loss():
    true_value = tf.variable(np.array([[1.2], [1],
                                      [1], [1]]))
    pred_value = tf.variable(np.array([[1.2], [0.1],
                                      [0], [-0.3]]))
    ...
    assert np.isclose(expected_loss, loss)
    true_value = tf.variable(np.array([[1.2], [1], [0.8],
                                      [1], [1], [0.8]]))
    pred_value = tf.variable(np.array([[1.2], [0.1], [-0.5],
                                      [0], [0], [-0.3]]))
...
def test_rank_crossentropy_loss():
    ...
    true_value = tf.variable(np.array([[1.], [0.],
                                      [0.], [1.]]))
    pred_value = tf.variable(np.array([[0.8], [0.1],
                                      [0.8], [0.1]]))
    expected_loss = (-np.log(softmax([0.8, 0.1])[0]) - np.log(
        softmax([0.8, 0.1])[1])) / 2
    loss = K.eval(losses.RankCrossEntropyLoss()(true_value, pred_value))
    assert np.isclose(expected_loss, loss)
    true_value = tf.variable(np.array([[1.], [0.], [0.],
                                      [0.], [1.], [0.]]))
    pred_value = tf.variable(np.array([[0.8], [0.1], [0.1],
                                      [0.8], [0.1], [0.1]]))
