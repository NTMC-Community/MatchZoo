...
class QueryDecision(BasicModel):
    ...
    def build(self):
        ...
        print('[layer]: Input\t[shape]: %s] \n%s' % (str(pair_feats.get_shape().as_list()), show_memory_use()))
        query_feats = Input(name='query_feats', shape=(self.config['query_feat_size'],))
        print('[layer]: Input\t[shape]: %s] \n%s' % (str(query_feats.get_shape().as_list()), show_memory_use()))
        ...
        print('[layer]: Lambda\t[shape]: %s] \n%s' % (str(q_average.get_shape().as_list()), show_memory_use()))
        ...
        print('[layer]: Reshape\t[shape]: %s] \n%s' % (str(q_pool1_flat.get_shape().as_list()), show_memory_use()))
        ...
        print('[layer]: Lambda\t[shape]: %s] \n%s' % (str(neg_q_rep.get_shape().as_list()), show_memory_use()))
        #average = Add()([q_average, neg_q_rep])
        average = Concatenate(axis=1)([q_average, neg_q_rep])
        print('[layer]: Concatenate\t[shape]: %s] \n%s' % (str(average.get_shape().as_list()), show_memory_use()))
        ...
        print('[layer]: Dense\t[shape]: %s] \n%s' % (str(attr.get_shape().as_list()), show_memory_use()))
        drop_attr = Dropout(0.5)(attr)
        print('[layer]: Dropout\t[shape]: %s] \n%s' % (str(drop_attr.get_shape().as_list()), show_memory_use()))

        pool1_flat_d = Dense(1)(pool1_flat)
        print('[layer]: Dense\t[shape]: %s] \n%s' % (str(pool1_flat_d.get_shape().as_list()), show_memory_use()))
        pair_feats_d = Dense(1)(pair_feats)
        print('[layer]: Dense\t[shape]: %s] \n%s' % (str(pair_feats_d.get_shape().as_list()), show_memory_use()))

        feat = Lambda(query_atten)([pool1_flat_d, pair_feats_d, drop_attr])
        print('[layer]: Lambda\t[shape]: %s] \n%s' % (str(feat.get_shape().as_list()), show_memory_use()))
        ...
        model = Model(inputs=[query, doc, pair_feats, query_feats], outputs=out_)
        return model
