...
class MatchLSTM(engine.BaseModel):
    ...
    def get_default_params(cls):
        ...
        params.add(engine.Param(
            'rnn_hidden_size', 256,
            hyper_space=engine.hyper_spaces.quniform(low=128, high=384, q=128)
        ))
        params.add(engine.Param(
            'fc_hidden_size', 200,
    ...
    def build(self):
        ...
        self.query_len = query.shape[1]
        self.doc_len = doc.shape[1]
        ...
        attn_layer = keras.layers.Lambda(self.attention)
    ...
    def attention(self, tensors):
        """Attention layer."""
        query, doc = tensors
        tensor_left = K.expand_dims(query, axis=2)
        tensor_right = K.expand_dims(doc, axis=1)
        tensor_left = K.repeat_elements(tensor_left, self.doc_len, 2)
        tensor_right = K.repeat_elements(tensor_right, self.query_len, 1)
