...
class PointGenerator(engine.BaseGenerator):
    ...
    def __init__(
        ...
        super(PointGenerator, self).__init__(batch_size, shuffle, is_train)
    ...
    def transform_data(self, inputs: DataPack):
        ...
        for idx, sample in enumerate(data.values):
            ...
            x0_id = sample[3]
            x1_id = sample[4]
            ctl = min(self.text_left_max_len, len(x0))
            ctr = min(self.text_right_max_len, len(x1))
            xx0 = np.zeros((self.text_left_max_len,), dtype=np.int32)
            xx1 = np.zeros((self.text_right_max_len,), dtype=np.int32)
            xx0[:] = fill_word   # padding
            xx1[:] = fill_word   # padding
            xx0[:ctl] = x0[:ctl]
            xx1[:ctr] = x1[:ctr]
