...
class SlidingWindowUnit(ProcessorUnit):
    ...
    SlidingWindow is used in :class:`CDSSMModel`.
    This processor is expected to execute after :class:`NgramLetterUnit`
    and before `Vocab` has been created.
...
class WordHashingWindowUnit(ProcessorUnit):
    ...
    The input of :class:`WordHashingWindowUnit` should be a list
    of `sliding letter_ngram` extracted from one document. The output
    of :class:`WordHashingWindowUnit` is the word-hashing representation
    of this document.
    ...
    def transform(
        ...
        """
        Transform list of :attr:`sliding words` into sliding
        word hashing layer.
